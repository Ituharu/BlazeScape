...
public class ShopHandler {
	...
	public void process() {
		...
		for(int i = 1; i <= TotalShops; i++) {
			...
			if (DidUpdate == true) {
				for (int k = 1; k < GameEngine.playerHandler.maxPlayers; k++) {
					if (GameEngine.playerHandler.players[k] != null) {
						if (GameEngine.playerHandler.players[k].IsShopping == true && GameEngine.playerHandler.players[k].MyShopID == i) {
							GameEngine.playerHandler.players[k].UpdateShop = true;
						}
					}
				}
				DidUpdate = false;

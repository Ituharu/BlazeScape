...
public class WalkingHandler {
	...
	public boolean traversable(int x, int y, int direction) {
        ...
        if (direction == 0 && (flag == 1 || flag == 4 || flag == 6 || flag == 7 || flag == 9 || flag == 11 || flag == 13 || flag == 14)) {
			...
			return false;
	} else if(x >= 2835 && x <= 2840 && y >= 3269 && y <= 3278) {
		return true;
	} else if(x >= 2829 && x <= 2833 && y >= 3264 && y <= 3276) {
		return true;
	} else if(x >= 2835 && x <= 2838 && y >= 3281 && y <= 3284) {
		return true;
	}
        return true;
	}
	...
	public void initialize() throws Exception {
		...
		RandomAccessFile raf = new RandomAccessFile("Data/WorldMap/lolmap.bin", "r");
